...
export default function Sidebar({
  ...
  function sourceTitle(src: string): string {
    return {
      esmfold: "Folded by ESMFold (single-sequence prediction)",
      alphafold: "AlphaFold DB structure (high quality, pre-computed by Google DeepMind)",
      pdb: "Experimental structure from RCSB Protein Data Bank",
      designed: "Generated by iterative design loop",
                ...
                {structures.map((s) => (
                  <div key={s.id} className={`structure-item ${s.id === activeId ? "active" : ""}`}>
                    <div className="structure-label" onClick={() => onSelectActive(s.id)}>
                      <div className="structure-row1">
                        <span className="structure-name">{s.label}</span>
                        <span className={`source-badge source-${s.source}`} title={sourceTitle(s.source)}>
